...
summary(accidents)
plot(accidents[,c("children_accidents", "children_exposure")], mar = c(0, 0, 0, 0), oma = c(3, 3, 1.5, .1),
...
plot(accidents[,c("seniors_accidents", "seniors_exposure")], mar = c(0, 0, 0, 0), oma = c(3, 3, 1.5, .1),
     main = "", xlab = "", ylab = "", ann = FALSE)
...
x <- seq(4.5, 6, by = 0.01)
plot(x, dgamma(x, aN, bN), type = "l", xlab = expression(mu), ylab = "",
...
plot(x, pgamma(x, aN, bN), type = "l", xlab = expression(mu), ylab = "",
...
for (i in seq_along(probs)) {
  lines(c(.9*min(x), qs[i]), c(probs[i], probs[i]), lty = 2, col = "dimgrey")
...
for (i in 2:length(aN)) lines(x, dgamma(x, aN[i], bN[i]), lty = i, col = i)
...
if (pdfplots) {
  pdf("2-1_4.pdf", width = 12, height = 8)
  par(mgp = c(1.6, .6, 0), mar = c(2, 2, 3, .5), lwd = 2)
}
par(mfrow = c(1, 2))
...
```{r, echo = -(1:2), fig.width = 8, fig.height = 4}
if (pdfplots) {
  pdf("2-1_5.pdf", width = 12, height = 8)
  par(mgp = c(1.6, .6, 0), mar = c(2, 2, 3, .5), lwd = 2)
}
par(mfrow = c(2, 2))
set.seed(1)

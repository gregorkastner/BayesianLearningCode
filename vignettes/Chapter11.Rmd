...
For Figure 11.3 we determine the prior distribution for the number of active covariates for $p=10$ covariates
```{r}
if (pdfplots) {
  pdf("11-2_2a.pdf", width = 6, height = 3)
}
require(ggplot2)
par(mfrow = c(1, 1), mar = c(2.5, 0, 1.5, .5),
    mgp = c(1.5, 0.5, 0))
colors=c("darkblue","darkred")

p=10
xx=0:p
pip_data<-data.frame(x=rep(xx,2),
               prob=c(dbinom(xx, size=p, prob=0.05), 
                      dbinom(xx, size=p, prob=0.5)),
               pi=c(rep("0.05",p+1),rep("0.5",p+1)) )

ggplot(pip_data, aes(x = x, y =prob, fill = pi)) +
  geom_bar(stat = "identity", width=0.6, position = "dodge")+
  scale_x_continuous(breaks =0:p)+
  scale_fill_manual(values=colors, 
                    guide=guide_legend( title = expression(pi))) +
  labs(x = "x", y = "P(X=x)")  +
  theme(text = element_text(size = 15)) 

```
and for $p=100$ covariates
```{r}
if (pdfplots) {
  pdf("11-2_2b.pdf", width = 6, height = 3)
}
p=100
xx=0:p
pip_data<-data.frame(x=rep(xx,2),
...
ggplot(pip_data, aes(x = x, y =prob, fill = pi)) +
  geom_bar(stat = "identity", width=0.6, position = "dodge")+
  scale_x_continuous(breaks=seq( from=0, to= p, by=10))+
  scale_fill_manual(values=colors, 
                    guide=guide_legend( title = expression(pi))) +
  labs(x = "x", y = "P(X=x)")  +
  theme(text = element_text(size = 15)) 

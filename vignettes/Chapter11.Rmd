...
```{r}
logmarlik_reg<-function(y,X, a0, A0, B0, c0, C0){
  N<-length(y)
  
  B0.inv <- diag(c(1/A0,1/B0))
  ...
  ldet <- N / 2 * log(2*pi) + lgamma(cN) - lgamma(c0) + c0 * log(C0) - cN * log(CN) +
    0.5 * det(BN, logarithm=TRUE) - 0.5*det(B0, logarithm=TRUE)
}

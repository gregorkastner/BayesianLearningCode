...
We first compute the marginaö likelihoods for the model of sales and log(sales)
and then apply the correction factor.
```{r}
X=covs.cen[, c("Screens","Budget" )]
gammas <- matrix(rep(1,2),nrow=1)

lM0 <- logmarlik_reg(y, X, a0=0, A0=100, B0=10, c0 = 2.5,C0 = 1.5,
                     gammas)
lMF_uncorr <- logmarlik_reg(log(y), X, a0=0, A0=10, B0=1, c0 = 2.5,C0 = 1.5,
                            gammas)
lMF<-lMF_uncorr- sum(log(y))
```
Next we compute the posterior model probabilities under the assumption that
...
```{r}
model_probs <- exp(c(lM0,lMF ) ) / sum(exp(lM0)+exp(lMF))
print(round(model_probs, 3))
```

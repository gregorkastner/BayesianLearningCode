...
```
We load the data and choose the covariates Budget, Screens and Comedy which we center at their means. We also define the indicators for all $2^3=8$ models that can be estimated
...
We then perform model selection also for the model with standardized covariates.
...
knitr::kable(cbind(gammas, logmarliks, model_probs),
             digits = c(rep(0, ncol(gammas)), 1, 3))
```
We find that results are different using original and standardized covariates. 
...
We  again start writing  a function that performs model selection using the
g-prior.
```{r}
...
```
As a  first exercise we apply the g-prior to the model with the covariates used above and also for the model with standardized covariates.
...
We now  perform variable selection for the movie data. We use all covariates from example 6.7  except for the categorical variable MMPA rating.
```{r}
...
We next determine the posterior distribution of the size of the models, i.e. the number of covariates. We plot the MCMC draws of the model size and its posterior distribution.
...
We now implement variable selection with the g-prior and a hierarchical prior
on the model space.
```{r}
...
We also determine the posterior distribution of the model space complexity, 
```{r}
if (pdfplots) {
...
a function for this task. As an alternative we could  implement this step in
...
```
 We apply this function to the model selection results under the uniform and the
 hierarchical model space  g-prior.
...
under both priors and provide results in  a table (left: uniform prior, right: hierarchical prior).
```{r}
...
We first compute the marginal likelihoods for the model of sales and that of log(sales)
in Example 6.3 and then we apply the correction factor.
```{r}
...
```
We then compute the posterior model probabilities under the assumption that
both models have equal prior probabilities (which hence can be omitted).
We find a  clear preference for the transformed model.
```{r}
...
```
We compute the average model complexity, the number of models visited during 
MCMC and the model visited most frequently.
```{r}
...
knitr::kable(cbind(gammas_unique,freq_gammas/M)[io[1:5],],
             digits=cbind(rep(0, p),4))
```
From the barplot of the PIPs we see that the irrelevant covariates have a 
PiP close to zero whereas the other covariates are included with a very high 
probability.
```{r}
if (pdfplots) {
...
### Example 11.18: Inflation data: Bayesian unit root testing with unknown model order

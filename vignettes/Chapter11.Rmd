...
```{r}
prob <- 1/dim(gammas)[1]
h <- exp(logmarliks-logmarliks[which.max(logmarliks)])
cM <- prob*sum(h)
model_probs <- prob*h/cM

require(xtable)
print(xtable(cbind(gammas,logmarliks, model_probs),
             digits=c(0,0,0,0,1,3)))
```

We next peform model selection also for the standardised covariates.
...
require(xtable)
print(xtable(cbind(gammas,logmarliks, model_probs), digits=c(0,0,0,0,1,3)))
```
...
We run the MH algorithm and compute the frequencies each model is visited.
...
print(xtable(cbind(gammas,model_freq, model_freq/M),

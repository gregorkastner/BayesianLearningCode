...
in Example 6.3. Then we apply the correction factor.
```{r}
...
To implement variable selection in a probit model we start by writing a function
for the log marginal likelihood.
```{r}
logmarlik_probreg<- function(z,X, a0, A0, B0, gamma){
   ...
   bNv <- solve(BNv.inv)%*% (crossprod(X_gamma, y) + B0v.inv%*% b0v )
   SSE <- as.numeric(crossprod(z) +  t(b0v) %*% B0v.inv %*% b0v -
                    t(bNv) %*% BNv.inv %*% bNv)
 
  lmarlik  <- (- N / 2) * log(2*pi) +  0.5 *( log(det(B0v.inv))-log(det(BNv.inv))   )-
                  SSE/2
  return(lmarlik)
}
```
We use this function to implement variable selection for the probit model.
```{r}
varsel_probit <- function(y, Xreg, a0=0, A0=1000, B0 = 1000,
...
M <- 20000 #/ mcmcspeedup
set.seed(seed)
res <- varsel_probit(y, X, M=20000)# B0=1000=A0

...
equal-tails intervals and highest posterior density (HPD) intervals. Note that R
is (generally) vectorized, so we can compute the equal-tails intervals without
using a loop.
...
b <- 1
aN <- c(40 + a, 4 + b)
bN <- c(400 - 40 + a, 400 - 4 + b)
names(aN) <- names(bN) <- c("City A", "Rural area")
...
for (i in seq_along(aN)) {
  ...
  plot(theta, dbeta(theta, aN[i], bN[i]), type = "l",
       ...
       main = bquote(.(names(aN[i])) * ":" ~
                     Pr({.(round(left[i,j], 4)) <= vartheta} <=
                     .(round(right[i,j], 4))) == .(gamma)))
...
# Equal-tails 95% credible intervals
leftET <- qbeta(alpha/2, aN, bN)
rightET <- qbeta(1 - alpha/2, aN, bN)

...
post_uniform_unnormalized <- counts + 1
...
post_lessinformative_unnormalized <-  counts + gamma0
...
```{r}
resfull <- cbind(post_uniform, post_lessinformative)
unseen <- counts == 0L
res <- rbind(resfull[!unseen,], UNSEEN = resfull[which(unseen)[1],])
knitr::kable(t(round(res, 3)))
```
...
post_informed_unnormalized <- counts + gamma0

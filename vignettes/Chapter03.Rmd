...
post_uniform_unnormalized <- table(merged)
...
$$
To compute this expectation, we can re-use the unnormalized posterior from above
by first subtracting the old pseudo-counts to retain the data, and then adding
the new pseudo-counts $\gamma_{0,1}, = \gamma_{0,2} = \dots = \gamma_{0,K} = 1/K$.

```{r}
N <- length(dat)
N0 <- 1
gamma0 <- rep(N0 / N, length(universe))
post_lessinformative_unnormalized <- post_uniform_unnormalized - 1 + gamma0
...
observations $N$
(this implies a data/prior ratio of 5 to 1). Each word
...
post_informed_unnormalized <- post_uniform_unnormalized - 1 + gamma0
post_informed <- post_informed_unnormalized / sum(post_informed_unnormalized)
```

Alternatively, we might want to prefer heavier smoothing with respect to the
...
post_informed_unnormalized2 <- post_uniform_unnormalized - 1 + gamma0
...
if (pdfplots) {
  pdf("3-3_1.pdf", width = 9, height = 9)
  par(mgp = c(1, .5, 0), mar = c(2.8, 2.2, 1.5, 0), lwd = 1)
}
par(mfrow = c(4, 1))
midpts <- barplot(post_uniform, las = 2)
points(midpts, rep(1/51, length(midpts)), col = "green")
title("Uniform prior")
...
points(midpts, freq / 10^8, col = "green")
title("Informed prior (light smoothing)")

midpts <- barplot(post_informed2, las = 2)
points(midpts, freq /10^8, col = "green")
title("Informed prior (heavy smoothing)")
```

In the top panel (based on the continuous uniform prior), we can nicely see the
effect of the *add-one smoothing*, whereas the middle panel shows almost no
smoothing, and unseen words are estimated to be extremely unlikely. The two bottom
panels show the posterior under the informed prior with light and heavy

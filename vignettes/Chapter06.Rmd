...
The prior variance of the intercept is set to the same value as in the semi-conjugate prior.
...
names(res_sigma2.hs) <- c("2.5%","post.mean", "97.5%")
...
par(mfrow=c(1,1), mgp = c(1.6, .6, 0), mar = c(2,2,2,0.1))

boxplot(sqrt(tau2.hs), outline = FALSE)
alpha <- 0.05
tau2.hs.trunc <- apply(tau2.hs, 2, function(x, alpha) x[x <= quantile(x, 1 - alpha)], alpha = alpha)
...
hist(beta.hs[,i], breaks = 100, xlab = "", ylab = "", main = c("Intercept", covs)[i])
}
plot.new()
for (i in seq_len(ncol(tau.hs.trunc.mirrored))) {
hist(tau.hs.trunc.mirrored[,i], breaks = 100, xlab = "", ylab = "", main = covs[i])
}
```

...
```
# Exercise 6.3.
We now predict the box the  sales on the opening weekend box office for a film with  an average number of \emph{Weeks} for a range of \emph{Screens} from
 to 
```{r}
 nf<-33
 X_new <- cbind(rep(1,nf),rep(0,nf),-21:11)
 colnames(X_new)<-colnames(X)
 
 ypred.hat=X_new%*%t(beta.hat)
 ypred.var <-sigma2.hat*(t(Xnew)%*%solve(crossprod(X))%*%Xnew+1))

 pred.int.sc<- apply(ypred.hat)

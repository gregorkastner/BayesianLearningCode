...
```
 Next we want to predict the box office sale for a movie with MPAA rating ``G'' or
  ``PG'', of no specific genre, with average values of \textit{Budget} and {Weeks}  as well as the sentiments and volumes of Twitter-posts set, however for different values of \textif{Screens}.
...
nf=3
 ...
 pred.int.sc<- apply(ypred.sc,1, quantile, probs=c(0.025, 0.975))
 
 ypred.hs<-X_new%*%t(beta.hs)+rnorm(sqrt(sigma2.hs))
 pred.int.hs<- apply(ypred.hs,1, quantile, probs=c(0.025, 0.975))
 ```

...
par(mfrow = c(3,4))
for (i in seq_len(d)) hist(beta.sc[,i], main = colnames(X)[i])
for (i in seq_len(d)) hist(beta.hs[,i], main = colnames(X)[i])
```
...
par(mfrow = c(3, 4))
for (i in seq_len(d)) plot(beta.sc[,i], type="l")
for (i in seq_len(d)) plot(beta.hs[,i], type="l")
```

Finally, we have a look at boxplots of the posterior distributions of the
local shrinkage parameters $\tau_j$ under the horseshoe prior.

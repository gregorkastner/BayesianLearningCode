...
weekend box office for a film with a mean value score of twitter post in weeks
4-6 and 1-3 before its start.
...
We plot the (univariate) marginal posterior distribution of the intercept
...
There is little difference to the improper prior for $B_0 = 10\textbf{I}$,
however we see shrinkage to zero for $B_0 = \textbf{I}$. The effect of the
prior isgiven by the weight matrix $\textbf{W}$, which is computed for the prior 
...
We now include not only the volume scores from twitter posts 4-6 and 1-3 weeks
before starting of the film, but use also the  information on the sentiments in
these time periods, the genres \textit{Comedy} and \textit{Thriller}, the MPAA
ratings, the budget, the number of weeks as well as screens the film was
planned to be shown. There is only one film with MPAA rating ``G'', hence we

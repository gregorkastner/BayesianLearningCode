...
We use movie data provided as "movie.xls" to   Bayesian analysis of a regression model. The data set is a preprocessed version of the one provided by Lehrer and Xi ().
...
#v2 <- c("Vol-21-27","Vol-14-20","Vol-7-13","Vol-4-6","Vol-1-3")
#movies[,v2] <- log(movies[,v2])
...
B0.inv=diag(c(0,rep(1/100,d)), nrow=d+1)
b0=rep(0,d+1)
...
for (m in 1:(burnin+M)){
    ...
    sigma2<-rinvgamma(1,shape=cN,rate=CN)
...
require(graphics)
boxplot.matrix(tau2.res,ylim=c(0,100))
```

...
N <- length(y)
hist(y, breaks = 50, main = "Histogram", xlab = "CHF/USD percentage log returns")
...
we can generate draws from it. For the Gaussian model, we simulate from
$\mathcal{G}^{-1}(c_N, C_N)$
and then, conditionally on these draws, from
$\mathcal{N}(\bar{y}, \sigma^2 / N)$.
...
alpha <- 0.005
ndraws <- 100000
sigma2draws <- rinvgamma(ndraws, cN, CN)
...
hist(qnormdraws, breaks = mybreaks, ylab = "", xlab = bquote(Q[.(alpha)]),
     main = bquote("Histogram of posterior" ~ .(100 * alpha) * "%-VaR draws under the" ~
                   N(0, sigma^2) ~ "likelihood assumption"), prob = TRUE)
points(quantile(y, alpha), 0, col = 2, lwd = 2)
hist(qtdraws, breaks = mybreaks, ylab = "", xlab = bquote(Q[.(alpha)]),
     main = bquote("Histogram of posterior" ~ .(100 * alpha) * "%-VaR draws under the" ~
                   t[.(nu)](0, sigma^2) ~ "likelihood assumption"), prob = TRUE)
points(quantile(y, alpha), 0, col = 2, lwd = 2)
```

...
it expects a scalar *sigma2* and a data vector $\mathbf{y}$. We can, however,
...
par(mfrow = c(1, 2))
nu <- 7
sigma2 <- seq(0.27, 0.38, length.out = 3000)
...
hist(qnormdraws, breaks = mybreaks, ylab = "", xlab = bquote(Q[.(alpha)]),
     main = bquote("Histogram of" ~ .(100 * alpha) * "%-VaR draws under the" ~
                     N(0, sigma^2) ~ "likelihood assumption."))
points(quantile(y, alpha), 0, col = 2)
hist(qtdraws, breaks = mybreaks, ylab = "", xlab = bquote(Q[.(alpha)]),
     main = bquote("Histogram of" ~ .(100 * alpha) * "%-VaR draws under the" ~
                     t[.(nu)](0, sigma^2) ~ "likelihood assumption."))
points(quantile(y, alpha), 0, col = 2)
```

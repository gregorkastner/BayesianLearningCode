...
post_unnormalized_nonvec <- function(sigma2, dat, nu, log = FALSE) {
  logdens <- -length(y) / 2 * log(sigma2) -
    (nu + 1) / 2 * sum(log(1 + dat^2 / (nu * sigma2))) - log(sigma2)
  if (log) logdens else exp(logdens)
}
post_unnormalized <- Vectorize(post_unnormalized_nonvec, "sigma2")

sigma2 <- seq(0.1, 1, length.out = 3000)
pdf_u <- post_unnormalized(sigma2, dat = y, nu = nu)
...
mudraws <- rnorm(ndraws, mean(y), sigma2draws/length(y))
qnormdraws <- qnorm(0.01, mudraws, sqrt(sigma2draws))
```

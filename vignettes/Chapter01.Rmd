...
```{r, include = FALSE}
par(mgp = c(1.6, .6, 0), mar = c(2, 2, 3, .5))
```
...
$$y|\theta_1 \sim Exp(\lambda),$$
and for the males that $$y|\theta_0 \sim G(\alpha, \beta),$$
...
title("Conditional densities of distance")
lines(x, dgamma(x, alpha, beta), col = "red")
```

Now, we compute the non-normalized posteriors for $y \in \{1, 2, 3, 6, 8\}$.
...
Finally, here are the posterior probabilities.

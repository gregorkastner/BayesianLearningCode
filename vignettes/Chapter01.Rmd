...
res <- rbind(PosteriorTheta1Unormalized / NormalizingConstant,
             PosteriorTheta0Unormalized / NormalizingConstant)
colnames(res) <- y
...
and $A^C$ are complementary.

Assuming we don't have any prior information about the probability of occurrence
...
curve(dexp(x, lambda), from = 0, to = 10, xlab = "Distance", ylab = "",
      main = "Conditional densities and non-normalized posteriors")
curve(dgamma(x, alpha, beta), col = "red", add = TRUE)
...
colnames(res) <- x
barplot(res, main = "Posterior probabilities", 
        col = c("black", "red"), xlab = "Distance", ylab = "Probabilities")
```

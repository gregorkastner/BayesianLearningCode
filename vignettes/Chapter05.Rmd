...
for (i in 1:6) {
  ...
  leftconf <- thetaML - qnorm(.975) * sqrt(thetaML * (1 - thetaML) / set$N[i])
  rightconf <- thetaML + qnorm(.975) * sqrt(thetaML * (1 - thetaML) / set$N[i])
...
res <- cbind(res[,1:2], asymptotic = colMeans(inconf), HPD = colMeans(inHPD))

...
Recap posterior from Chapter 4.
```{r}
...
Read in data again, and simulate from the posterior by Monte Carlo.
...
ybar <- mean(y)
s2 <- var(y) * (N - 1) / N
sigma2draws <- 1 / rgamma(ndraws, (N - 1) / 2, N * s2 / 2)
mudraws <- rnorm(ndraws, mean(y), sqrt(sigma2draws / N))
draws <- data.frame(mu = mudraws, sigma2draws = sigma2draws)
```

Obtain the functional values and keep the top `r 100 * (1 - alpha)` percent.

```{r}
dens <- posterior(draws$mu, draws$sigma2, ybar, s2, N)
HPDdraws <- draws[rank(dens) > floor(ndraws * alpha),]
```
...
```{r}
toplot <- sample.int(nrow(HPDdraws), 10000)
plot(draws[toplot,], pch = 16, col = rgb(0, 0, 0, .05), cex = 2)
hullind <- chull(HPDdraws)
polygon(HPDdraws[hullind,], col = rgb(1, 0, 0, .2))
```

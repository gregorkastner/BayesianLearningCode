...
res <- cbind(thetatrue = c(thetatrue[1], NA, NA, thetatrue[2], NA, NA),
             ...
             SN = as.vector(SN),
             thetaML = thetaML,
             thetameanB11 = postmean[1:6],
             thetamodeB11 = postmode[1:6],
             thetameanB42 = postmean[7:12],
             thetamodeB42 = postmode[7:12])
...
## Table 5.3: Coverage probabilities
For an analysis of the (frequentist) coverage probabilities, we simulate
...
for (i in 1:6) {
  ...
  leftconf <- thetaML - qnorm(cutoff) * sqrt(thetaML * (1 - thetaML) / set$N[i])
  rightconf <- thetaML + qnorm(cutoff) * sqrt(thetaML * (1 - thetaML) / set$N[i])
  ...
  leftequal <- qbeta(1 - cutoff, aN, bN)
  rightequal <- qbeta(cutoff, aN, bN)
  inequal[, i] <- leftequal <= set$thetatrue[i] & set$thetatrue[i] <= rightequal
  lenequal[, i] <- rightequal - leftequal
}

res <- cbind(res[,1:2],
             asy_coverage = colMeans(inconf),
             HPD_coverage = colMeans(inHPD),
             equal_coverage = colMeans(inequal),
             asy_length = colMeans(lenconf),
             HPD_length = colMeans(lenHPD),
             equal_length = colMeans(lenequal))

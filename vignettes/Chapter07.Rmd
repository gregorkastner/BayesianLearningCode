...
mcmcspeedup <- 10L # default: 1; set this to something higher for fewer draws
```
...
```{r, echo = -c(1:2)}
if (pdfplots) {
  pdf("7-1_1.pdf", width = 9, height = 4.5)
  par(mar = c(2.6, 1.5, 1.5, .1), mgp = c(1.5, .5, 0), lwd = 2, font.main = 1)
}
par(mfrow = c(2, 2))
...
```{r, echo = -c(1:2)}
if (pdfplots) {
  pdf("7-2_12.pdf", width = 9, height = 5)
}
...
Let us investigate trace plots and empirical autocorrelation functions of the
...
1. Ignore the part coming from $y_0$ and only moment-match the shifted beta
prior. This is currently described in the manuscript (auxprior = analytical1)

2. Use a part coming from the determinant of the density of $y_0$, resulting in
a shifted $\mathcal{B}(a^\phi + 0.5, b^\phi + 0.5)$ (auxprior = analytical2)

3. Numerically approximate mode and curvature of the distribution at every
iteration (auxprior = numerical)
...
First, we load the data and compute the growth of the absolute log returns.

```{r, echo = -c(1:2), fig.width = 8, fig.height = 4}
if (pdfplots) {
  pdf("7-3_1.pdf", width = 8, height = 4)
}
par(mfcol = c(2, 3), mar = c(2.5, 2.5, 1.5, .1), mgp = c(1.4, .5, 0),
    font.main = 1)
...
We now implement the MCMC sampler for fitting an MA(1) model, where we treat the
latent state $\epsilon_0 \sim \mathcal{N}(0, \sigma^2)$ as unknown. For the
innovation variance, we assume an inverse gamma prior, and $\theta$ is chosen to be a priori uniform on $[-1, 1]$.
...
if (pdfplots) {
  pdf("7-3_2.pdf", width = 8, height = 5)
}
...
As above, now with a truncated Gaussian proposal for the random walk MH algorithm.
...
```{r, echo = -c(1:2), fig.width = 8, fig.height = 3}
if (pdfplots) {
  pdf("7-3_3.pdf", width = 8, height = 5)
}
par(mfcol = c(2, 3), mar = c(2.5, 2.5, 1.5, .1), mgp = c(1.5, .5, 0), lwd = 2,
    font.main = 1)
for (i in seq_along(cthetas2)) {
...
We repeat the exercise above once more, but now use a random walk proposal on
$\log(1 + \theta) - \log(1 - \theta)$.
...
```{r, echo = -c(1:2), fig.width = 8, fig.height = 4}
par(mfcol = c(1, 2), mar = c(2.5, 2.5, 1.5, .1), mgp = c(1.5, .5, 0), lwd = 2,
    font.main = 1)
qqplot(thetas[, 2], thetas2[, 2])
abline(c(0, 1), col = 2)
...
if (pdfplots) {
  ...
  par(mar = c(2.6, 2.6, 1.5, .5), mgp = c(1.5, .5, 0), lwd = 2, font.main = 1)
}
par(mfrow = c(1, 3))
set.seed(1)
...
if (pdfplots) {
  ...
  par(mar = c(1.5, 2, 1.5, .1), mgp = c(1.5, .5, 0), lwd = 2, font.main = 1)
}
par(mfrow = c(1, 2))
...
```{r, echo = -c(1:2)}
if (pdfplots) {
  pdf("7-4_4.pdf", width = 10, height = 3.5)
  par(mar = c(2.6, 2.6, 1.5, .1), mgp = c(1.5, .5, 0), lwd = 2, font.main = 1)
}
par(mfrow = c(1, 2))
...
if (pdfplots) {
  ...
  par(mar = c(1.5, 1.5, 1.5, .1), mgp = c(1.5, .5, 0), lwd = 2, font.main = 1)
}
par(mfrow = c(1, 2))
M <- 1000

...
hist(res3$phi, breaks = mybreaks, col = rgb(1, 1, 0, .2),
     freq = FALSE, add = TRUE)
lines(mybreaks[mybreaks <= 1], col = 2, lty = 3, lwd = 3,
      dbetarescaled(mybreaks[mybreaks <= 1], 1, 1))
lines(mybreaks[mybreaks <= 1], col = 3, lty = 4, lwd = 3,
...
```{r, echo = -c(1:2)}
if (pdfplots) {
  pdf("7-4_1.pdf", width = 12, height = 5)
  par(mar = c(2.6, 1.5, 1.5, .1), mgp = c(1.5, .5, 0), lwd = 2)
}
par(mfrow = c(1, 3))
set.seed(1)
...
We transform the data to obtain for each worker a matrix which
contains the number of transitions from one class to the other, the
matrices with values $N_{i,hk}$.
...
```{r, echo = -c(1:2)}
if (pdfplots) {
  pdf("7-4_2.pdf", width = 9, height = 3)
  par(mar = c(2.6, 1.5, 1.5, .1), mgp = c(1.5, .5, 0), lwd = 2)
}
par(mfrow = c(1, 2))
library("corrplot")

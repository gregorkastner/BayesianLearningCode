...
```{r, echo = -c(1:2)}
if (pdfplots) {
  pdf("7-4_3.pdf", width = 9, height = 3)
  par(mar = c(2.6, 1.5, 1.5, .1), mgp = c(1.5, .5, 0), lwd = 2)
}
par(mfrow = c(1, 2))
...
in wage category 2, i.e., $\eta_0 = (0 0.7 0.3 0 0 0)$. We compare the
...
(right-hand side) in year $t = 10$ for females (left-hand posterior in
both plots) and males (right-hand posterior in both plots).

```{r, echo = -c(1:2)}
if (pdfplots) {
  pdf("7-4_5.pdf", width = 9, height = 3)
  par(mar = c(2.6, 1.5, 1.5, .1), mgp = c(1.5, .5, 0), lwd = 2)
}
par(mfrow = c(1, 2))
M <- 1000

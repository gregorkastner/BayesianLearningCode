...
First, we estimate an AR(0), i.e., an intercept-only, model.
...
par(mfrow = c(1, 1), mar = c(2.6, 1.5, 1.5, .1), mgp = c(1.5, .5, 0), lwd = 2)
mu <- sigma2 <- matrix(NA_real_, draws, 5, dimnames = list(NULL, order = 0:4))
...
for (p in 1:4) {
  ...
  sigma2[, as.character(p)] <- res[[p]]$sigma2s /
    (1 - rowSums(res[[p]]$betas[, 2:(p + 1), drop = FALSE]))^2
}
  
plot(density(mu[, "0"], bw = "SJ", adj = 2), xlim = range(mu), ylab = "",
       main = "Posterior of the marginal mean", xlab = expression(mu))
for (p in 1:4) lines(density(mu[, as.character(p)], bw = "SJ", adj = 2),
                     col = p + 1, lty = p + 1)
legend("topright", paste0("p = ", 0:4), col = 1:5, lty = 1:5)

plot(density(sigma2[, "0"], bw = "SJ", adj = 2), xlim = range(sigma2), ylab = "",
       main = "Posterior of the marginal variance", xlab = expression(sigma^2))
for (p in 1:4) lines(density(sigma2[, as.character(p)], bw = "SJ", adj = 2),
                     col = p + 1, lty = p + 1)
legend("topright", paste0("p = ", 0:4), col = 1:5, lty = 1:5)
```

...
```{r, echo = -c(1:2), fig.width = 8, fig.height = 3}
if (pdfplots) {
  pdf("7-3_3.pdf", width = 8, height = 3)
}
...
```{r, echo = -c(1:2), fig.width = 8, fig.height = 3}
if (pdfplots) {
  pdf("7-3_4.pdf", width = 8, height = 3)
}
par(mfrow = c(1, 4), mar = c(2.5, 2.5, 1.5, .1), mgp = c(1.5, .5, 0), lwd = 2)
for (p in 1:4)
  toplot[, p] <- 1 - rowSums(ardat[[p]]$betas[, 2:(p + 1), drop = FALSE])
...
for (i in seq_along(cthetas)) {
  ...
  for (m in seq_len(ndraws + nburn)) {
    ...
    pL <- pnorm(-1, theta, cthetas[i])
    pU <- pnorm(1, theta, cthetas[i])
    norm <- pU - pL
    U <- runif(1)
    thetaprop <- qnorm(pL + U * norm, theta, cthetas[i])
    epsprop <- filter(dat, -thetaprop, "recursive", init = eps0)
    normprop <- diff(pnorm(c(-1, 1), thetaprop, cthetas[i]))
...
for (i in seq_along(cthetas)) {
  ...
  for (m in seq_len(ndraws + nburn)) {
    ...
    thetatransprop <- rnorm(1, trans(theta), cthetas[i])

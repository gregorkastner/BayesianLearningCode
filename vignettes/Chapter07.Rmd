...
```{r}
perclogret <- 100 * (log(dat[-1]) - log(dat[-length(dat)]))
```
...
par(mfrow = c(1,2))
ts.plot(perclogret, main = "U.S. GDP percentage log returns")
acf(perclogret, lag = 6, main = "")
...
ARdesignmatrix <- function(dat, p = 1) {
  ...
  Xy[, 1] <- 1
  for (i in seq_len(p)) {
    Xy[, i + 1] <- perclogret[(p + 1 - i) : (length(perclogret) - i)]
  }
  Xy
}
```

Now we obtain draws for four AR models under the improper prior.
...
for (p in 1:4) {
  y <- tail(perclogret, -p)
  Xy <- ARdesignmatrix(perclogret, p)
  res[[p]] <- regression(y, Xy, prior = "improper", M = draws)
}
```

...
par(mfrow = c(1, 3), mar = c(2.5, 2.5, 1.2, .1), mgp = c(1.5, .5, 0), lwd = 2)
...
for (m in seq_len(ndraws + nburn)) {
  ...
  tmp <- y - X %*% beta
  sigma2 <- rinvgamma(1, c0 + length(y) / 2, C0 + crossprod(tmp))
  
  # Step (c): Draw the intercept
  BT <- 1 / (1 / B0 + length(y) / sigma2)
  bT <- BT * (b0 / B0 + (y[1] - phi * y0 + sum(y[-1] - phi * y[-length(y)])) / sigma2)
  ...
  if (-1 < phiprop & phiprop < 1) {
    logR <- dbetarescaled(phiprop, a0, A0, log = TRUE) -
      dbetarescaled(phi, a0, A0, log = TRUE) +
...
priormean <- 2 * a0 / (a0 + A0) - 1
priorvar <- 4 * (a0 * A0) / ((a0 + A0)^2 * (a0 + A0 + 1))
...
To conclude, we compare the draws from the two samplers; they should yield
draws from the same distribution, irrespective of the mixing. We graphically
...
hist(res1$phi, breaks = 50, col = rgb(0, 0, 1, .3),
     main = "Histogram", xlab = expression(phi), freq = FALSE)
hist(res2$phi, breaks = 50, col = rgb(1, 0, 0, .3), freq = FALSE, add = TRUE)
...
and the posterior under the stationary-enforcing shifted beta prior.

```{r, echo = -c(1:2)}
if (pdfplots) {
  pdf("7-2_12.pdf", width = 9, height = 4)
}
par(mfrow = c(1, 1), mar = c(2.5, 2.5, 1.5, .1), mgp = c(1.5, .5, 0), lwd = 1.5)
mybreaks <- seq(floor(100 * .99 * min(res1$phi)) / 100,
...
hist(res2$phi, breaks = mybreaks, col = rgb(1, 0, 0, .2),
     freq = FALSE, add = TRUE)
lines(mybreaks[mybreaks <= 1], dbetarescaled(mybreaks[mybreaks <= 1], a0, A0),
      col = 2, lty = 3, lwd = 3)
legend("topright",
       c("Unrestricted posterior", "Post-processed posterior",
         "Beta prior posterior"),
       fill = rgb(c(0, 0, 1), c(1, 0, 0), c(0, 1, 0), .2))
legend("topleft", "Beta prior", col = 2, lty = 3, lwd = 3)
```
...
if (pdfplots) {
  pdf("7-2_x1.pdf", width = 8, height = 4)
}
...
```{r, echo = -c(1:2), fig.width = 8, fig.height = 3}
if (pdfplots) {
  pdf("7-2_x2.pdf", width = 8, height = 4)
}

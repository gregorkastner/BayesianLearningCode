...
## Figure 7.3
We now plot the posterior of the intercept under all those priors.

```{r, echo = -c(1:2)}
if (pdfplots) {
  pdf("7-2_3.pdf", width = 8, height = 5)
}
par(mfrow = c(1, 2), mar = c(2.5, 1.5, 1.2, .1), mgp = c(1.5, .5, 0))
plot(density(res_improper$betas[, 1], bw = "SJ", adj = 2),
     xlim = range(res_improper$betas[, 1], res_semi_1$betas[, 1], res_semi_2$betas[, 1]),
     main = "Posterior of the intercept", xlab = expression(zeta), ylab = "")
lines(density(res_semi_1$betas[, 1], bw = "SJ", adj = 2), col = 2, lty = 2)
lines(density(res_semi_2$betas[, 1], bw = "SJ", adj = 2), col = 3, lty = 3)

plot(density(res_improper$betas[, 1], bw = "SJ", adj = 2),
     xlim = range(res_improper$betas[, 1], res_conj_1$betas[, 1], res_conj_2$betas[, 1]),
     main = "Posterior of the intercept", xlab = expression(zeta), ylab = "")
lines(density(res_conj_1$betas[, 1], bw = "SJ", adj = 2), col = 2, lty = 2)
lines(density(res_conj_2$betas[, 1], bw = "SJ", adj = 2), col = 3, lty = 3)
```

## Figure 7.4
And the marginal posteriors for the autoregressive coefficients.

```{r, echo = -c(1:2)}
if (pdfplots) {
  pdf("7-2_4.pdf", width = 8, height = 5)
}
par(mfrow = c(3, 2), mar = c(2.5, 1.5, .5, .1), mgp = c(1.5, .5, 0))
for (i in 1:3) {
  dens_improper <- density(res_improper$betas[, i + 1], bw = "SJ", adj = 2)
  dens_semi_1 <- density(res_semi_1$betas[, i + 1], bw = "SJ", adj = 2)
  dens_semi_2 <- density(res_semi_2$betas[, i + 1], bw = "SJ", adj = 2)
  ...
  plot(dens_improper, xlim = range(dens_improper$x, dens_semi_1$x, dens_semi_2$x),
       ...
       main = "", xlab = bquote(phi[.(i)]), ylab = "")
  lines(dens_semi_1, col = 2, lty = 2)
  lines(dens_semi_2, col = 3, lty = 3)
  
  dens_improper <- density(res_improper$betas[, i + 1], bw = "SJ", adj = 2)
  dens_conj_1 <- density(res_conj_1$betas[, i + 1], bw = "SJ", adj = 2)
  dens_conj_2 <- density(res_conj_2$betas[, i + 1], bw = "SJ", adj = 2)

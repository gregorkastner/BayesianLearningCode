...
par(mfrow = c(1, 3), mar = c(2.5, 2.5, 1.2, .1), mgp = c(1.5, .5, 0), lwd = 1.5)
ar1draws <- res[[1]]$betas[,2]
ar2draws <- res[[2]]$betas[,2:3]
ar3draws <- res[[3]]$betas[,2:4]
...
Assume that the values at $t_1 = 62$ and $t_2 = 72$ are missing.
...
We visualize the observed and the missing values (black lines), alongside the
unobservable true values (red circles).
...
where <- seq(missing[1] - 3, missing[2] + 3)
...
par(mfrow = c(2, 2), mar = c(2.7, 1.5, 1.4, .1), mgp = c(1.5, .5, 0), lwd = 1.5)
for (i in 1:4) {
  if (i <= 3) {
    if (i == 1) name <- bquote(zeta) else name <- bquote(phi[.(i - 1)])
    dens_improper <- density(res_improper$betas[, i], bw = "SJ", adj = 2)

...
Finally, we repeat the exercise for a higher number of different values of
$N_0$.

```{r, echo = -1}
if (pdfplots) {
  pdf("10-1_3.pdf", width = 8, height = 4)
}
par(mar = c(2.5, 2.5, 1.5, .5), mgp = c(1.6, .6, 0))
N0 <- 10^seq(1, 10, by = 0.1)
N <- length(y)
...
abline(h = logSD[length(logSD)], lty = 3)
```
...
```{r, echo = -c(1:2)}
if (pdfplots) {
  pdf("10-4_1.pdf", width = 10, height = 5)
}

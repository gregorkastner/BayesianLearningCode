...
To predict the time series of children killed or seriously injured in Linz 
use the functions defined for estimating a Poisson regression model.
...
from October 1994  (i.e when the  legal intervention giving priority to pedestrians  became effective) using only data before that time point. Hence we estimate the model in Example 8.8. with an intercept and a holiday effect using only the information up to September 1993.
...
We see that the prediction interval is much too wide indicating that 
an intervention effect is required.

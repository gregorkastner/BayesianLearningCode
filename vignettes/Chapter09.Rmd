...
We verify that a 95% posterior predictive interval is given by [1, 9] using the
...
We determine the 95% posterior predictive interval using the quantile function.
...
We use a sampling-based approach to obtain draws from the posterior predictive
...
```{r, echo = -c(1:2)}
if (pdfplots) {
  pdf("9-5_2.pdf", width = 7, height = 2.5)
  par(mar = c(2.5, 2, 1.5, .5), mgp = c(1.6, .6, 0))
}
par(mfrow = c(1, 4))
horizon <- 8
...
```{r, echo = -c(1:2)}
if (pdfplots) {
  pdf("9-5_3.pdf", width = 10, height = 4)
  par(mar = c(2.5, 2.5, 1.5, .5), mgp = c(1.6, .6, 0))
}
quants <- apply(yfs, 2, quantile, c(.05, .1, .25, .5, .75, .9, .95))
par(mfrow = c(1, 1))
...
Let us compute the probability of seeing negative growth rates a least once in
eight quarters. Not that this is highly nonlinear.
...
Another example of a nonlinear function of the predicted log returns is the
level, which, given our simulation-based approach, is easy to compute.
...
```{r, echo = -c(1:2)}
if (pdfplots) {
  pdf("9-5_4.pdf", width = 10, height = 4)
  par(mar = c(2.5, 2.5, 1.5, .5), mgp = c(1.6, .6, 0))
}
quants <- apply(gdpfs, 1, quantile, c(.05, .1, .25, .5, .75, .9, .95))
par(mfrow = c(1, 1))
...
```{r}
loglik <- function(y, theta, sigma02 = var(y), y02 = var(y)) {
  pars <- unrestricted2restricted(theta)
  sigma2 <- rep(NA_real_, length(y))

  sigma2[1] <- pars[1] + pars[2] * y02 + pars[3] * sigma02
  for (i in 2:length(y)) {
...
We are now ready to apply the sampler to our USD-CHF exchange rate percentage
log returns. After running the sampler, we need to transform back to our
original parameters.
...
Let us visualize the parameters and the posterior mean of the conditional
standard deviations (along with the data).
...
We now turn towards prediction, which can be done iteratively. We define a
function that predicts a number of steps ahead.
...
Alternatively, we might want to do sequential one-step-ahead predictions. Note
that this amounts to re-estimating the model very often, thus we reduce the
number of posterior draws and the burn-in. To speed up convergence, we
initialize the sampler at the posterior means from above.
...
lines(pastwindow, colMeans(sqrt(restrain$sigma2[, pastwindow])))
lines(predwindow, colMeans(sqrt(predstatic$sigma2)))
...
abline(v = cutoff, lty = 3)
lines(pastwindow, colMeans(sqrt(restrain$sigma2[, pastwindow])))
lines(pastwindow[c(1, length(pastwindow))],
      rep(mean(sqrt(restrain$para$alpha0 / (1 - restrain$para$alpha1 -
                                              restrain$para$gamma1))), 2),
      lty = 2)
lines(predwindow, predsigmamean)
lines(predwindow, longrunsdmean, lty = 2)
for (i in seq_along(thesequants)) {
...
legend("bottomleft", legend = c("Data", "Conditional std. dev.",
       ...
       col = c("gray", "black", "black", "red"), lty = c(1, 1, 2, 1))
```

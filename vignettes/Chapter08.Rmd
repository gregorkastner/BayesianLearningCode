...
```
We see that the tighter prior shrinks the estimates to zero, estimated ESSs are
higher and estimated IFs are lower.
...
Correspondingly the autocorrelation of the draws are much lower under the
tighter prior. 
...
To check the MCMC algorithm for correctness, we extend the sampler by adding
sampling the data from the prior as a further sampling step.
...
We  run the sampler and check the effective sample size of the draws of the
regression effects and the heterogeneity parameter.
...
Neither the QQ-plot nor the
Kolmogorov-Smirnov test shows any evidence that the sampler is incorrect.
...
0.001 for the heterogeneity parameter which indicates that the sampler is
erroneous. 
...
To check the correctness of the Gibbs sampler we focus on the overdispersion
$\frac{\mu_i^2}{\alpha}$ for $x_i = 0$ and $x-i = 1$, which we compute from 
...
```
There is no evidence that the sampler is incorrect.
...
We finally compare bivariate scatterplots of the regression effects $\beta_0$ and $\beta_1$ and the heterogeneity parameter $\alpha$ for both samplers.
```{r, echo = -c(1:3)}
if (pdfplots) {
...
To model trend and seasonality, we set up an appropriate
design matrix. Leveraging the fact the the data is a `ts` object,

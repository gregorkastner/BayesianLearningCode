...
and employed as dependent variable, and use as covariates the variables
female (binary), age18 (quantitative, centered at 18 years), wcollar (binary), and
unemployed in 1997 (binary). The baseline person is hence an 18 year
old male blue collar worker who was employed in 1997.
...
knitr::kable(res_eff)
```
The effective sample size is at least `r min(ess)` for every regression effect, and hence all inefficiency factors are below `r max(ineff)`.
...
under the Normal prior with mean $\mathbf{0}$ and variance matrix $10000\mathbf{I}$ and run the sampler for $M=20000$ iterations after a burnin of 1000.

From the plot of the  ACF of the draws we see that autocorrelations 
...
remain very high even at lag 40 for the covariate effect. Hence we have a high ESS for
the intercept (`r ess.qus2[1]`) and a low for the covariate effect (`r ess.qus2[2]`), resulting in an inefficiency 
...
posteriors in case of separation and with a tighter prior we can shrink coefficients
to zero.

# Exeample 8.6: Complete seperation: analysis under an informative prior
...
We again use the Normaö prior with mean $\mathbf{\zerov}$ and covariance matrix $10000 \mathbf{I}$           on the regression
...
The risk of being unemployed  1998 for a male blue  collar worker of age 18, who 
was employed 1997 is very low with a value of `r p_unemploy_base_logit` and the risk is even lower for a white collar  worker. It is higher for females, increases with age and is particularly high for persons who were unemployed 1997.
...
As the logistic distribution has a variance of $\pi^2/3$ compared to 1 for the
standard Normal distribution, the regression effects in the logit model  are
absolutely larger than those in the probit model. However any probability computed from the two models will be very close, e.g.,  the estimated probability to be unemployed for a baseline person is  `r p_unemploy_base_probit` in the probit model (compared to `r p_unemploy_base_logit` in the logit model).

By multiplying the estimated coefficients in the probit model by
$\pi/\sqrt{3}$ we can  compare them to the estimates of the logit model and we see that there is not much difference.  
...
the acceptance rate deteroriates, when the dimension of regression effects $d$ increases.
...
We use again rather flat normal independence  prior $\mathcal{N}(\mathbf{0}, 100 \mathbf{I})$ on the regression effects. First we  determine the parameters of a Normal proposal distribution.
...
res1$accept
```
The baseline risk is  2.7 per 10000 children months and it is  lower in holiday months as well as after the intervention.  
...
```
In Model 2 the estimated baseline risk is similar that from model 1  with 2.3 dead or seriously injured children per 10 000 at risk.
Also the estimated  intervention effect is very similar  in both models, indicating a reduction of the risk by a factor of 0.72 in model 2 (compared to a factor of 0.70 in model 1). The posterior mean for the linear trend effect is zero, and hence we can conclude that there is no linear trend. 

With respect to monthly effects, we see that the effect has positive sign,  which means  an increased risk in the months April, June and October, whereas also the upper limit of the 95\% HPD interval  is negative, i.e. risk is loweron  the holiday months July and August.
As noted above,  due to the sum to zero constraint the effect of December results as the negative sum of the effects of all other months.   

```{r}
res2$accept
```
The  acceptance rate is `r round(res1$accept,2)` for the smaller model 1 with three parameters, but  only `r round(res2$accept,2)` for model 2, where 14 parameters

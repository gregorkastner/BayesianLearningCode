...
table(y,x)
```
We estímate the  model parameters and plot the acf of the draws. Again the autocorrelations remain high even for lag 35.
...
Next we set uo the independence Metropolis Hastings algorithm and estimate the model parameters.
```{r}
...
poisson <- function(y, X,  b0 = 0, B0 = 100,qmean,qvar,
  ...
  beta<-as.vector(rmvnorm(1, mean=qmean, sigma=qvar))
  
  for (m in (1:M)){
    beta.old <- beta
    beta.proposed <- as.vector(rmvnorm(1, mean=qmean, sigma=qvar))
    
    # compute log proposal density at proposed and old value
    lq_proposed <- dmvnorm(beta.proposed,mean=qmean, sigma=qvar, log=TRUE)
    lq_old  <- dmvnorm(beta.old, mean=qmean, sigma=qvar, log=TRUE)
    
    # compute log prior  of proposed and old value
    lpri_proposed <- dmvnorm(beta.proposed, mean=b0, sigma=B0, log=TRUE )
    lpri_old  <- dmvnorm(beta.old,  mean=b0, sigma=B0, log=TRUE )
...
res.poisson1<- rbind(apply(res1$beta.post,2, res.mcmc),exp(colMeans(res1$beta.post)))
rownames(res.poisson1) <- c("2.5%", "Mean", "97.5%", "exp(Mean)") 
colnames(res.poisson1) <- colnames(X)

knitr::kable(round(t(res.poisson1),3))
print(res1$accept)
```

We then fit an alternative model with intercept, intervention effect, linear trend and saisonal dummy variables. 
...
Define the prior parameters and compute parameters of the proposal distribution 
```{r}
b0=rep(0,d)
B0 <- diag(rep(100, d), nrow = d) 
B0.inv<- diag(rep(1/100, d), nrow = d) 
...
res.poisson2<- rbind(apply(res2$beta.post,2, res.mcmc),
                     exp(colMeans(res2$beta.post)))
rownames(res.poisson2) <- c("2.5%", "Mean", "97.5%", "exp(Mean)") 
colnames(res.poisson2) <- colnames(X.large)

knitr::kable(round(t(res.poisson2),3))
print(res2$accept)

...
This is different when the response variable  contains either only few or very many successes. To illustrate this issue we use data where in N=100 trials  only  1 success   or 999 sucesses are observed..
...
$N=100$ observations with 10 successes and 990 failures. 
...
acf(betas[,2])
```
Next we consider the case of quasi-seperation. We simulate again $N=100$ observations with 10 successes and 990 failures. 
We add a binary predictor $x$ where for $x=1$  only successes but  for $x=0$ successes as well as failures occur.
...
table(y,x)
```
A plot of the acf of the draws shows low autocorrelation for the intercept but again very high autocorrelations for the covariate effect.

...
```
Next, we  determine the estimated risk of unemployment for a baseline person, i.e. a 18 year
old male blue collar worker who was employed in 1997, using the posterior mean estimate of the intercept.

```{r}
(p_unemploy_base_probit <- round(pnorm(res_probit.labour[1, 2]),4))
```

The estimated risk to be unemployed in 1998 for a baseline person is  very low with a value of `r p_unemploy_base_probit` and  even lower for a white collar worker. This risk is  higher for a female,
...
$\mathcal{N}(\mathbf{0}, \mathbf{I})$. This prior distribution encodes the prior believe that  $\beta_0$ and $\beta_1$ are in the interval $(-1.96, 1.96)$$ with probability $0.95$.
...
was employed 1997 is very low with a value of `r p_unemploy_base_logit` and the risk is even lower for a white collar  worker. It is higher for females, increases with age and is particularly high for persons who were unemployed 1997.
 ...
 As discussed above, while the signs of the covariate effects can be interpreted in the same way for the probit and the logit model, their  numerical value will differ due to  the different scale of the link function.

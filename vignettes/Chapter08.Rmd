...
```
We see that with the tighter prior the estimates are more shrunk to zero, estimates ESSs are higher and inflation p
...
$\pi/\sqrt{3}$ we can  compare them to those of the logit model and we see that there is not much difference.  
...
We use a rather flat normal prior $\Normal{mathbf{0}, 100 \mathbf{100}}$ on the regression effects and first determine the parameters of a Normal proposal distribution.
...
res1$accept
```
The baseline risk is `r base_risk' and it is lower in holiday months as well as after the intervention.  
...
```
In Model 2 the estimated baseline risk is similar that from model 1  with `r round(exp(res.poisson2[1,2]),2)*10^4` dead or seriously injured children per 10 000 at risk.
...
With respect to monthly effects, we see that there is a positive effect, i.e. an increased risk for a child in Linz to be killed or seriously injured in the months April, June and October, whereas it is considerably smaller in the holiday months July and August.
As noted above,  due to the sum to zero constraint the effect of December results as the negative of the sum of the effects of all other months.   
...
We specify the prior on $\alpha$ as the Gamma distribution with shape
2 and rate 0.5 and use both samplers to estimate the model parameters.

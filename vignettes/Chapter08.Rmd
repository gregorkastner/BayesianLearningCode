...
```
We model the binary variable unemployment and use as covariates the variables female (binary), wcollar (binary),  age18 (quantitative, centered at 18 years) and unemployed in 1997 (binary).
...
We plot histograms of the draws and their autocorrelation function. Obviously the 
draws from the posterior distribution are autocorrelated but this autocorrelation
vanishes after some lags.
...
However the sampler has  problems when the response variable  contain either only few or very many successes as autocorrelation of the draws then increases. To illustrate this  We use data where out of 1000 observations 
 1 respectively 999 are successes.
...
acf(betas2)
```
In these two cases the autocorrelations of the draws decrease only very slowly.
...
betas1 <- probit(y1,X,b0=0, B0=0.1)
plot(betas1,type="l", main = "", xlab = "", ylab = "")
acf(betas1)

betas2 <- probit(y2,X,b0=0, B0=0.1) 
plot(betas2,type="l", main = "", xlab = "", ylab = "")
acf(betas2)
```
With a tighter prior the ACF decreases faster, however the posterior mean is closer to zero.

...
The estimated ESS is at least `r min(ESS)` for every regression effect, and hence all estimated inefficiency factors (IFs) are below `r max(IF)`.
...
High autocorrelation in MCMC draws for probit models occurs not only when
...
From the plot of the empirical ACF of the draws we see that autocorrelations 
...
the acceptance rate deteriorates, when the dimension of regression effects $d$ increases.
...
With respect to monthly effects, we see that the effect has positive sign,  which means  an increased risk in the months April, June and October, whereas also the upper limit of the 95\% HPD interval  is negative, i.e. risk is lower on  the holiday months July and August.
...
The two sampler differ, however particularly w.r.t. the inefficiency

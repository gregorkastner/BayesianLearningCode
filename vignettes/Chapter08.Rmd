...
```{r}
sample_alpha <- function(y, mu, phi, pri.alpha, alpha.old, 
                       c.alpha, full.gibbs){
...
negbin<- function(y,X,e, b0,B0, pri.alpha,c.alpha,
  ...
  for (m in seq_len(burnin + M)){
    ...
    # sample alpha
    alpha.draw<-sample_alpha (y,mu=e*exp(linpred),phi,pri.alpha,alpha,
                              c.alpha,full.gibbs)
...
And then we run the partially marginalized Gibbs sampler under the same prior.   
```{r}
...
negbin_check_abc <- function(X,e, b0,B0, pri.alpha,c.alpha,
  ...
  for (m in seq_len(burnin + M)){
    ...
    # Step b
    alpha.draw <- sample_alpha(y,e*exp(linpred), phi, pri.alpha,
...
negbin_check_cba <- function(X,e, b0,B0,pri.alpha,c.alpha,
  ...
  for (m in seq_len(burnin + M)){
    ...
    # Step b
    alpha.draw <- sample_alpha(y,e*exp(linpred), phi,pri.alpha,alpha,
                               c.alpha,full.gibbs)
    alpha<- alpha.draw$alpha

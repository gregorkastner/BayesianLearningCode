...
sucesses and failures. We illustrate this issue also on simulated data,
with $N=1000$ observations where 500 successes and 500 failures are observed. 
We add a binary predictor $x$ where for $x=1$ we observe only successes.
```{r}
N <- 1000
...
table(y,x)
```
We estimate the model parameters and plot the acf of the draws. 
...
the autocorrelation of the covariate effect remains very high.
With a flat prior the 

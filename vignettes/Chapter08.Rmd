...
# xtable(res.poisson1, caption= "Count Data: estimation results for Model 1", label="res.count1", digits=3)
 ...
 # xtable(res.poisson2, caption= "Count Data: estimation results for Model 2", label="res.count2", digits
```
...
negbin <- function(y, X, e, b0 = 0, B0 = 100, qmean, qvar, pri.alpha,
                    full.gibbs= FALSE, burnin = 1000L, M = 5000L) {
  ...
  # set starting values
  beta <- as.vector(mvtnorm::rmvnorm(1, mean = qmean, sigma = qvar))
  alpha.old=pri.alpha$shape/pri.alpha$rate
  phi=rep(1.,N)
  ...
  lq_proposed <- dmvnorm(beta.proposed,mean=qmean, sigma=qvar, log=TRUE)
           #dmvt(delta,df=3, sigma=mod.var,log=TRUE)
  lq_old  <- dmvnorm(beta.old, mean=qmean, sigma=qvar, log=TRUE)
             #dmvt(beta.old-mod.mean,df=3, sigma=mod.var,log=TRUE)
  ...
  if (full.gibbs){
     ...
     log_acc_alpha <-sum(dgamma(phi, shape=alpha.proposed, rate=alpha.proposed,
                               log=TRUE)) -
                     sum(dgamma(phi, shape=alpha.old, rate=alpha.old,
                              log=TRUE)) +
    ...
    llik_alpha.proposed=sum(dnbinom(y, size=alpha.proposed, mu=exposed*exp(linpred),
                                    log = TRUE))
    llik_alpha.old=sum(dnbinom(y, size=alpha.old, mu=exposed*exp(linpred),
                               log = TRUE))
  ...
  phi<- rgamma (N,shape=alpha+y, rate=alpha+exposed*exp(linpred))

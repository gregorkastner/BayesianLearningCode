...
`r round(mean(ESS_sep))`, resulting in estimated IFs of about 
`r round(mean(IF_sep), -2)`.
...
(`r round(ESS_qus2[1])`) and a low for the covariate effect 
(`r round(ESS_qus2[2])`), resulting in an estimated IF of 
`r round(IF_qus2[1])` for the intercept, but of `r round(IF_qus2[2])` 
...
2, where `r ncol(X_large)` regression coefficient parameters have to
be estimated.
...
We first write a function to sample  the parameter $\alpha$ for both, the full
Gibbs sampler and the partially marginalised Gibbs sampler using a log random walk proposal.
...
```
Then we combine the sampling steps for sampling $\beta$, $\alpha$ and $\phi$ in
a gibbs sampler.
...
We  run the sampler and check the effective sample size of the draws of the regression 
effects and the heterogeneity parameter.
...
alpha_prior <- qgamma(ppoints(M), shape = pri_alpha$shape,
                      rate = pri_alpha$rate)
```
Next we thin the sampled values by a factor of 100 to get (almost) uncorrelated 
draws and make a QQ-plot of the values from
the prior and the thinned draws from posterior.

```{r, echo = -c(1:3)}
if (pdfplots) {
   pdf("8-2_1.pdf", width = 8, height = 8)
}
par(mfrow = c(2, 2), mar = c(2.5, 2.5, 1.5, .1), mgp = c(1.5, .5, 0), lwd = 1.5)
...
Kolmogorov-Smirnov test is larger than 0.05 we
can conclude that the sampler is correct. 

Next we implement  sampling alpha by using a proposal ratio of 1, which would be
correct for a random walk proposal, however is wrong for a log-random walk 
proposal.
...
We use this wrong  sampling step in our Gibbs sampler and check for 
its correctness. 
...
```{r, echo = -c(1:3)}
if (pdfplots) {
  pdf("8-2_2.pdf", width = 8, height = 8)
}
par(mfrow = c(2, 2), mar = c(2.5, 2.5, 1.5, .1), mgp = c(1.5, .5, 0), lwd = 1.5)
...
for the heterogeneity parameter which indicates that the sampler is erroneous. 
...
```{r, echo = -c(1:3)}
if (pdfplots) {
  pdf("8-2_3.pdf", width = 8, height = 8)
}
par(mfrow = c(2, 2), mar = c(2.5, 2.5, 1.5, .1), mgp = c(1.5, .5, 0), lwd = 1.5)
...
Again the points of the QQ-plot lie close to the
identity line and the p-value of the Kolmogorov-Smirnov test is larger than 
0.05 and thus we conclude that the sampler is correct. 


```{r, echo = -c(1:3)}
if (pdfplots) {
  pdf("8-2_4.pdf", width = 8, height = 8)
}
par(mfrow = c(2, 2), mar = c(2.5, 2.5, 1.5, .1), mgp = c(1.5, .5, 0), lwd = 1.5)
...
text(4, 0.2, paste0('KS-test: p-value= ', round(ks.a$p.value,4)))
```                                                     
This is not the case for the partial marginalized sampler with the wrong 
sampling step for the heterogeneity parameter: we get a Q-Q plot which deviates
considerably from the lidentity line and a p-value of the Kolmogorov-Smirnov
statistics of 0, which indicates that the sampler is not correct.

## Example 8.12 : Changing the order of the sampling steps
In this example we check whether changing the order of the sampling 
steps to (c)- (b)-(a) (instead of (a)-(b)-(c) ) still yields a correct Gibbs
sampler.
We first set up the sampler.
...
```
We define the size of the sample and use one binary covariate which has the 
value zero for half of the data and one for the other half
half of the data and use Normal prior for the regression parameters with mean
$(0.5,2)$ and covariance matrix $0.2 \mathbf{I}$  and a Gamma prior with shape 5 
and rate 10 for the heterogeneity parameter $\alpha$. 
...
```{r, echo = -c(1:3)}
if (pdfplots) {
  pdf("8-2_5.pdf", width = 8, height = 4)
}
...
Now we run the partially marginalised Gibbs sampler, first with correct order
of sampling steps.
```{r, echo = -c(1:3)}
if (pdfplots) {
  pdf("8-2_6.pdf", width = 8, height = 4)
}
par(mfrow = c(1, 2), mar = c(2.5, 2.5, 1.5, .1), mgp = c(1.5, .5, 0), lwd = 1.5)
...
mu2=exp(res_partial$beta_post[,1]+res_check_partial$beta_post[,2])
...
We compare the scatterplots of the regression effects $\beta_0$ and $\beta_1$
versus the heterogeneity parameter $\alpha$
```{r, echo = -c(1:3)}
if (pdfplots) {
  pdf("8-2_7.pdf", width = 8, height = 4)
}
par(mfrow = c(3, 2), mar = c(2.5, 2.5, 1.5, .1), mgp = c(1.5, .5, 0), lwd = 1.5)
plot(res_check_full$beta_post[,1],res_check_full$alpha_post)
plot(res_partial$beta_post[,1],res_partial$alpha_post)
plot(res_check_partial$beta_post[,1],res_check_partial$alpha_post)
plot(res_check_full$beta_post[,2],res_check_full$alpha_post)
plot(res_partial$beta_post[,2],res_partial$alpha_post)
plot(res_check_partial$beta_post[,21],res_check_partial$alpha_post)
```
...
We begin by loading the data and visualizing them as a time series plot. In
what is to follow, we restrict our analysis to the years 2003 through 2012,
i.e., a time span of 10 years or 120 observations.
...
an appropriate design matrix. Leveraging the fact the the data is a `ts` object,
we can do this very easily using `time` and `cycle`. 
...
```{r, echo = -c(1:2)}
if (pdfplots) {
  pdf("8-3_5.pdf", width = 8, height = 6.5)
}
par(mfrow = c(2, 1), mar = c(2.5, 1.5, 1.5, .5), mgp = c(1.5, .5, 0), lwd = 1.5)
...
We see a generally good fit but also notice some potential autocorrelation.
Thus, we move forward by including an autoregressive coefficient.
...
```{r, echo = -c(1:2)}
if (pdfplots) {
  pdf("8-3_6.pdf", width = 7, height = 5)
}
